...
describe('AI Services Performance Tests', () => {
    ...
    describe('Response Time Tests', () => {
        ...
        it('should generate images within acceptable time limits', async () => {
            ...
            // Mock the API call
            vi.spyOn(imagenService, 'generateImage').mockResolvedValue({
                images: [{
        ...
        it('should generate videos within acceptable time limits', async () => {
            ...
            // Mock the API call
            vi.spyOn(veoService, 'generateVideo').mockResolvedValue({
                video: {
                    uri: 'mock-video-uri'
                }
            });
    ...
    describe('Concurrent Request Handling', () => {
        it('should handle multiple caption requests concurrently', async () => {
            ...
            const requests = Array.from({ length: 5 }, (_, i) => geminiService.generateCaption({
    ...
    describe('Error Recovery Performance', () => {
        it('should handle errors quickly without blocking', async () => {
            ...
            vi.spyOn(geminiService, 'generateContent').mockRejectedValue(new Error('API Error'));
            const startTime = performance.now();
            try {
                await geminiService.generateCaption({
        ...
        it('should recover from rate limiting efficiently', async () => {
            const geminiService = new GeminiService();
            let callCount = 0;
            vi.spyOn(geminiService, 'generateContent').mockImplementation(() => {
                callCount++;
                if (callCount <= 2) {
