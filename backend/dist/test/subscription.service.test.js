...
describe('SubscriptionService', () => {
    ...
    describe('createCheckoutSession', () => {
        it('should create checkout session for pro tier', async () => {
            const result = await SubscriptionService.createCheckoutSession(testUser.id, 'pro', 'https://example.com/success', 'https://example.com/cancel');
            expect(result.sessionId).toBe('cs_test123');
            expect(result.url).toBe('https://checkout.stripe.com/test');
        });
        it('should throw error for free tier', async () => {
            await expect(SubscriptionService.createCheckoutSession(testUser.id, 'free', 'https://example.com/success', 'https://example.com/cancel')).rejects.toThrow('Cannot create checkout session for free tier');
        });
    });
