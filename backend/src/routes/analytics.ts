...
router.get('/overview', requireAuth, async (req, res) => {
  try {
    ...
    if (startDate) {
      conditions.push(gte(analytics.recordedAt, new Date(startDate as string)));
    }
    
    if (endDate) {
      conditions.push(lte(analytics.recordedAt, new Date(endDate as string)));
    }
    
    if (platform) {
      conditions.push(eq(posts.platform, platform as string));
    }
    ...
    const platformQuery = db
      .select({
        ...
        count: sql<number>`count(*)`,
        totalViews: sql<number>`coalesce(sum(case when ${analytics.metricType} = 'views' then ${analytics.metricValue} else 0 end), 0)`,
        totalLikes: sql<number>`coalesce(sum(case when ${analytics.metricType} = 'likes' then ${analytics.metricValue} else 0 end), 0)`,
      })
      .from(posts)
...
router.get('/performance', requireAuth, async (req, res) => {
  try {
    ...
    if (metricType) {
      conditions.push(eq(analytics.metricType, metricType as string));
    }

    // Get time series data grouped by date
    const timeSeriesQuery = db
      .select({
        date: sql<string>`date(${analytics.recordedAt})`,
        platform: posts.platform,
        totalValue: sql<number>`sum(${analytics.metricValue})`,
        count: sql<number>`count(*)`,
      })
    ...
    const trendsQuery = db
      ...
      .where(
        and(
          ...
          gte(analytics.recordedAt, daysAgo)
        )
      )
...
router.post('/metrics', requireAuth, async (req, res) => {
  try {
    ...
    const analyticsData = metrics.map((metric: { type: string; value: number }) => ({
      postId,
