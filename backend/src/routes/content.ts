...
router.post('/generate-caption', 
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Validate platform
      const validPlatforms: Platform[] = ['instagram', 'tiktok', 'youtube', 'linkedin', 'twitter'];
...
router.get('/library', 
  ...
  try {
    ...
    if (contentType && typeof contentType === 'string') {
      const validContentTypes: ContentType[] = ['caption', 'image', 'video'];
      ...
      data: {
        ...
        filters: {
          platform: (platform && typeof platform === 'string' && 
                     ['instagram', 'tiktok', 'youtube', 'linkedin', 'twitter'].includes(platform as Platform)) 
                     ? platform : null,
          contentType: (contentType && typeof contentType === 'string' && 
                       ['caption', 'image', 'video'].includes(contentType as ContentType)) 
                       ? contentType : null,
          sortBy: sortField,
          sortOrder: order,
        },
...
function getPlatformImageDimensions(platform: Platform, aspectRatio?: string): { width: number; height: number } {
...
function getPlatformImageFormats(platform: Platform): string[] {
  const formats = {
    instagram: ['JPG', 'PNG'],
    tiktok: ['JPG', 'PNG'],
    youtube: ['JPG', 'PNG', 'GIF'],
...
function getPlatformVideoSpecs(platform: Platform, requestedDuration?: number): {
  duration: number;
  maxDuration: number;
  aspectRatio: string;
} {
...
function getPlatformVideoFormats(platform: Platform): string[] {
  const formats = {
    instagram: ['MP4', 'MOV'],
    tiktok: ['MP4', 'MOV'],
    youtube: ['MP4', 'MOV', 'AVI'],
