...
export class VeoService {
   ...
   * Get platform-specific video specifications
   */
  private getPlatformVideoSpecs(platform: string, aspectRatio?: string) {
    const platformSpecs = {
      instagram: {
    ...
    return platformSpec[ratio as keyof typeof platformSpec] || platformSpec.landscape;
  }

  /**
   * Build enhanced prompts for video generation
   */
  private buildVideoPrompt(options: VeoGenerationOptions): string {
  ...
  private async mockVideoGeneration(
    prompt: string, 
    specs: { width: number; height: number; maxDuration: number; fps: number }, 
    duration?: number
  ): Promise<VeoResponse> {
   ...
   */
  private validateRequest(options: VeoGenerationOptions): { valid: boolean; error?: string } {
   ...
   */
  async getGenerationStatus(jobId: string): Promise<{ status: 'pending' | 'processing' | 'completed' | 'failed'; progress?: number }> {
